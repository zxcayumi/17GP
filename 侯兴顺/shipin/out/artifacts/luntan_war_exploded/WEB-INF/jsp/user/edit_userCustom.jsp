...
function addRow(){ //读取最后一行的行号，存放在txtLastIndex文本框中 
	...
	//添加列内容 
	newColumn.innerHTML = "<input name='_itemValue' type='text' class='form-text' size='20' /><input type='hidden' name='itemKey' value=''>"; 
...
<TABLE class="t-table" cellSpacing="1" cellPadding="2" width="100%" border="0">
  ...
  <TR>
    <TD class="t-label t-label-h" width="12%">选择类型：</TD>
    <TD class="t-content" width="88%" colSpan="3">
	    <select class="form-select" id="chooseType" name="chooseType" onChange="selectType(this);" disabled="disabled">
	    ...
	    <TD class="t-content" width="85%" colSpan="3">
	    	...
	    	<table width="250px" border="0" cellpadding="0" cellspacing="0" id="input_table" >  
	   			...
	   			<c:forEach items="${itemValue_map}" var="itemValue" varStatus="status">
					...
					<c:set var="itemValue_count" value="${status.count+1}"></c:set>
   				</c:forEach>
			
		   	</table> 
		   	<input type='hidden' id='inputLastIndex' value="${itemValue_count}" /> 
