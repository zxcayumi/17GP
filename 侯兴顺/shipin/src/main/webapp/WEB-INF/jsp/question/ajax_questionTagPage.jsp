...
<TABLE class="t-table" cellSpacing="1" cellPadding="2" width="100%" border="0">
	    ...
	    <TD class="t-content" colSpan="5" height="28px">
	        ...
	        <span style="float:left;">&nbsp;<a href="javascript:tagPage(1,0)">全部标签</a></span>
			<span style="float:left; margin-top: 1px;">
			&nbsp;
			<c:forEach items="${navigation}" var="navigationMap" varStatus="status">
				>>&nbsp;<a href="javascript:tagPage(1,'${navigationMap.key}')">${navigationMap.value}</a>
			</c:forEach>
			</span>
		</TD>
	</TR>
  ...
  <TBODY class="t-list-tbody" align="center">
   <c:forEach items="${pageView.records}" var="entry" varStatus="status"> 
	   <c:if test="${status.count%2!=0 }"><TR></c:if>
	  	<TD>
	  		<c:if test="${entry.childNodeNumber ==0}"> <INPUT TYPE="radio" name="type" value="${entry.id}" onclick="javascript:addTag('${entry.id}','${entry.name }')"></c:if>
		</TD>
	    <TD align="left">
			<c:if test="${entry.childNodeNumber >0}"><a href="javascript:tagPage(1,'${entry.id}')">&nbsp;${entry.name }</a></c:if>
