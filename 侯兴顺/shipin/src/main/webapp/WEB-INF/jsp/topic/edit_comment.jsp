...
<script>
	...
	KindEditor.ready(function(K) {
		...
		//	autoHeightMode : true,//值为true，并引入autoheight.js插件时自动调整高度
			...
			uploadJson :"${config:url(pageContext.request)}control/comment/manage.htm?method=uploadImage&topicId="+topicId+"&userName=${comment.userName}&isStaff=${comment.isStaff}&${_csrf.parameterName}=${_csrf.token}",//指定浏览远程图片的服务器端程序
...
function sureSubmit(){
	...
	post_request(function(value){
		...
		"${config:url(pageContext.request)}control/comment/manage${config:suffix()}?method=edit&timestamp=" + new Date().getTime(), true,parameter);
  ...
  <TR>
   ...
   <TR>
    <TD class="t-content" style="height: 30px;">
    	状态：<label><input name="status" type="radio" value="10" <c:if test="${comment.status == 10}"> checked="checked"</c:if>>待审核</label>
    	<label><input name="status" type="radio" value="20" <c:if test="${comment.status == 20}"> checked="checked"</c:if>>已发布</label>
    </TD>
   </TR>
    <TR>
    <TD class="t-content" >
    	<textarea id="content" name="content" style="width:99%;height:300px;visibility:hidden;">${comment.content}</textarea>
    </TD>
   </TR>
	<TR>
    <TD class="t-button">
