...
<head>
    ...
    <link rel="shortcut icon" type="image/x-icon" href="${config:url(pageContext.request)}backstage/images/favicon.ico"
          media="screen"/>
                        ...
                        <ul>
                            <li class="nav-list-item">
                                <div class="pos-rel" title="网站首页">
                                    <div class="nav-list-title2">
                                        <a class="link-icon" hidefocus="true" href="" target="_blank"><i
                                                class="fa fa-home2" style="color:#ff9e00;"></i></a>
                                    </div>
                                </div>
                            </li>
                            <li class="nav-list-item">
                                <div class="pos-rel" title="后台首页">
                                    <div class="nav-list-title2">
                                        <div class="link-icon"
                                             onclick="createWindow('home','首页','control/center/home${config:suffix()}');return false;">
                            ...
                            <li class="nav-list-item hide">
                                ...
                                <div class="nav-pop-list">
                                    <ul class="ul-navlist">
                                        <li onclick="createWindow('A_6_0','员工列表','control/staff/manage${config:suffix()}?method=editStaff&userId=${sysUsers.userId }&page=');return false;">
                                            <i class="fa fa-user-cog"></i>个人设置
                                        </li>
                                        <li onClick="javascript:submitLogout();return false;"><i
                                                class="fa fa-sign-out-alt"
                                                style="font-size: 13px;margin-right: 5px;"></i>退出登录
                                        </li>
                                    </ul>
                                </div>
                                <form:form id="logout" action="${config:url(pageContext.request)}admin/logout"
...
//左边显示/隐藏菜单
    ...
    function additem(intitle, parentid, inkey) {
        if (parentid >= 0 && parentid <= config.titlelist.length) {
            ...
            config.itemlist[parentid][config.itemlist[parentid].length] = new theitem(intitle, insort, inkey, 0);
...
//	window.top.frames['mainFrame'].location=url+suffix;
    ...
    function initConfig() {
        ...
        /*	t=config.addtitle('问答管理','内容管理',1);
            config.additem('问题列表',t,'
        ${config:url(pageContext.request)}control/question/list
        ${config:suffix()}');
	config.additem('标签列表',t,'
        ${config:url(pageContext.request)}control/questionTag/list
        ${config:suffix()}');
	config.additem('全部待审核问题',t,'
        ${config:url(pageContext.request)}control/question/allAuditQuestion
        ${config:suffix()}');
	config.additem('全部待审核答案',t,'
        ${config:url(pageContext.request)}control/question/allAuditAnswer
        ${config:suffix()}');
	config.additem('全部待审核回复',t,'
        ${config:url(pageContext.request)}control/question/allAuditAnswerReply
        ${config:suffix()}');
	config.additem('问题搜索',t,'
        ${config:url(pageContext.request)}control/question/search
        ${config:suffix()}');
	*/
        //config.additem('批量操作',t,'');
        /*	t=config.addtitle('留言管理','内容管理',1);
            config.additem('留言列表',t,'
        ${config:url(pageContext.request)}control/feedback/list
        ${config:suffix()}');
	t=config.addtitle('友情链接管理','内容管理',1);
	config.additem('友情链接列表',t,'
        ${config:url(pageContext.request)}control/links/list
        ${config:suffix()}');
