...
//删除回复
function deleteAnswerReply(answerReplyId){
	  ...
	  <TR >
	    ...
	    <TD width="15%">
	    	<c:if test="${entry.status == 10}">
	    		<A onclick="javascript:if(window.confirm('确定通过吗? ')){auditAnswerReply('${entry.id}');return false;}else{return false};" hidefocus="true" href="#" ondragstart= "return false">立即审核</A>
	    	</c:if>
	    	<a href="${config:url(pageContext.request)}control/question/manage${config:suffix()}?method=view&questionId=${entry.questionId}&answerId=${entry.answerId}&userName=${param.userName}&id=${param.id}&queryState=${param.queryState}&jumpStatus=${param.jumpStatus}&userPage=${param.userPage}&answerReplyPage=${param.page}&origin=30">查看</a>
	    	<a href="#" onclick="javascript:if(window.confirm('确定删除吗? ')){deleteAnswerReply('${entry.id}');return false;}else{return false};" hidefocus="true" ondragstart= "return false">删除</a>
	    </TD>
	  </TR>
	 </c:forEach>
  </TBODY>
