  ...
  <TBODY class="t-list-tbody" align="center">
   <c:forEach items="${pageView.records}" var="entry" varStatus="status"> 
	   <c:if test="${status.count%2!=0 }"><TR></c:if>
	  	<TD>
	  		<c:if test="${entry.childNodeNumber ==0}"> <INPUT TYPE="radio" name="type" value="${entry.id}" onclick="javascript:addHelpType('${entry.id}','${entry.name }','${tableName}')"></c:if>
		</TD>
	    <TD align="left">
			<c:if test="${entry.childNodeNumber >0}"><a href="javascript:helpTypePage(1,'${entry.id}','${tableName}')">&nbsp;${entry.name }</a></c:if>
