...
//还原私信
function reductionPrivateMessage(id,userId){
	var parameter = "";
	parameter += "&privateMessageId="+id;
	...
	if(parameter.indexOf("&") == 0){
   	...
   	post_request(function(value){
		...
		"${config:url(pageContext.request)}control/privateMessage/manage${config:suffix()}?method=reductionPrivateMessage&timestamp=" + new Date().getTime(), true,parameter);
		...
		<TR>
		    <TD class="t-content" colSpan="5" height="28px">
				...
				<span style="float:left">
					&nbsp;>>&nbsp;<a href="${config:url(pageContext.request)}control/privateMessage/manage${config:suffix()}?method=privateMessageList&id=${param.id}&userName=${param.userName}&queryState=${param.queryState}&jumpStatus=${param.jumpStatus}&userPage=${param.userPage}" >全部私信</a>
				</span>
				<span style="float:left">
					&nbsp;>>&nbsp;与&nbsp;<span style="color: orange;">${chatUser.userName}</span>&nbsp;对话
				</span>
			</TD>
		</TR>
	</TBODY>
	  ...
	  <TR >
	    ...
	    <TD width="10%">
	    	<c:if test="${entry.status >100}">
	    		<a href="#" onclick="javascript:if(window.confirm('确定还原吗? ')){reductionPrivateMessage('${entry.id}','${entry.userId}');return false;}else{return false};" hidefocus="true" ondragstart= "return false" title="还原用户已删除的私信">还原</a>
	    	</c:if>
	    </TD>
	  </TR>
	 </c:forEach>
