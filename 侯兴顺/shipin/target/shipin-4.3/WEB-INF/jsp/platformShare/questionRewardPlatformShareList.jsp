   ...
   <c:forEach items="${pageView.records}" var="entry">
	  <TR >
	    ...
	    <TD width="13%">
	     ...
	     <TD width="13%">
	    	${entry.answerUserName}<c:if test="${entry.staff}"><span style="color: green;"> 员工</span></c:if>
	    </TD>
	    <TD width="19%">
	    	${entry.answerUserShareRunningNumber}
	    </TD>
	    <TD width="10%">${entry.platformShareProportion}%</TD>
	    <TD width="10%">${entry.totalAmount}</TD>
	    <TD width="10%" >${entry.shareAmount}</TD>
	    <TD width="12%">
	    	<fmt:formatDate value="${entry.adoptionTime}"  pattern="yyyy-MM-dd HH:mm:ss"/>
	    </TD>
