...
function sureSubmit(){
	...
	var parameter = "";
	//回复Id
	var answerReplyId = document.getElementById("answerReplyId");
	if(answerReplyId != null){
		parameter += "&answerReplyId="+answerReplyId.value;
	}
	//状态
	...
	post_request(function(value){
		...
		"${config:url(pageContext.request)}control/answer/manage${config:suffix()}?method=editAnswerReply&timestamp=" + new Date().getTime(), true,parameter);
...
<form:form>
<input type="hidden" id="answerReplyId" value="${param.answerReplyId}">
<DIV class="d-box">
...
<TABLE class="t-table" cellSpacing="1" cellPadding="2" width="100%" border="0">
  ...
  <TR>
    <TD class="t-content" height="25px">
    	${answerReply.ip}&nbsp;&nbsp;${answerReply.ipAddress}
    </TD>
   </TR>
   <TR>
    <TD class="t-content" style="height: 30px;">
    	<label><input name="status" type="radio" value="10" <c:if test="${answerReply.status == 10}"> checked="checked"</c:if>>待审核</label>
    	<label><input name="status" type="radio" value="20" <c:if test="${answerReply.status == 20}"> checked="checked"</c:if>>已发布</label>
    </TD>
   </TR>
    <TR>
    <TD class="t-content" >
    	<textarea class="form-textarea" id="content" name="content" style="width:99%;height:300px;">${answerReply.content}</textarea>
    </TD>
   </TR>
   
	<TR>
    <TD class="t-button">
    	<SPAN id="answerReplyId_error" name="error" class="span-text" ></SPAN>
