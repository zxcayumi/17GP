  ...
  <TBODY class="t-list-tbody" align="center">
   <c:forEach items="${pageView.records}" var="entry">
	  <TR >
	    ...
	    <TD width="40%">
	    	<fmt:formatDate value="${entry.cancelTime}"  pattern="yyyy-MM-dd HH:mm:ss"/>
	    </TD>
	    <TD width="30%">
	    	<c:if test="${entry.hideTagType == 10}">输入密码</c:if>
	    	<c:if test="${entry.hideTagType == 40}">积分购买</c:if>
	    	<c:if test="${entry.hideTagType == 50}">余额购买</c:if>
	    </TD>
	  </TR>
	 </c:forEach>
  </TBODY>
