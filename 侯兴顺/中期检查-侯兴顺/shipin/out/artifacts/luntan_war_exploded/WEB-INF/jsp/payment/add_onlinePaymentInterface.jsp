...
<DIV class="d-box">
<div class="d-button">
	<input class="functionButton" type="button" onClick="javascript:window.location.href='${config:url(pageContext.request)}control/onlinePaymentInterface/list${config:suffix()}'" value="返回">
</div>
	  ...
	  <TR>
	    ...
	    <TD class="t-content" width="85%" colSpan="3">
	    	<select class="form-select" id="interfaceProduct" name="interfaceProduct" onChange="selectInterfaceProduct()">
	    		...
	    		<c:forEach items="${paymentInterfaceProductList}" var="paymentInterfaceProduct">
	    			<c:set var="paymentInterfaceProductValue" value=""></c:set>
	    			<c:if test="${paymentInterfaceProduct == 1}">
	    				<c:set var="paymentInterfaceProductValue" value="支付宝即时到账"></c:set>
	    			</c:if>
	    			<c:if test="${paymentInterfaceProduct == 4}">
	    				<c:set var="paymentInterfaceProductValue" value="支付宝手机网站(alipay.trade.wap.pay)"></c:set>
	    			</c:if>
	    			
	    			<option value="${paymentInterfaceProduct}" <c:if test="${onlinePaymentInterface.interfaceProduct == paymentInterfaceProduct}"> selected='selected'</c:if>>${paymentInterfaceProductValue }</option>
	    		</c:forEach>
	    	</select>
	    	<c:if test="${fn:length(paymentInterfaceProductList) == 0}">
	  ...
	  <TBODY id="style_1" <c:if test="${onlinePaymentInterface.interfaceProduct ne '1'}"> style="DISPLAY: none"</c:if>>
		  <TR>
			    ...
			    <TD class="t-content" width="85%" colSpan="3">
			    	...
			    	<input name="direct_app_id"  type="text" class="form-text" size="50" maxlength="200" value="${alipayDirect.app_id }" />
